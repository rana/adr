...
impl UspsClient {
    ...
    pub async fn standardize_address(&mut self, adr: &mut Address) -> Result<()> {
        ...
        .await
        {
            ...
            Err(err) => {
                eprintln!("standardize_address: err:{adr:?}");
                eprintln!("standardize_address: err:{err:?}");
                ...
                if let usps_addresses_sdk::apis::Error::ResponseError(ref inr) = err {
                    if inr.status == StatusCode::BAD_REQUEST {
                        // 2004 N CLEVELAND ST,,DAYTON,TX,77536
                        // Invalid zip code.
                        // Re-fetch address without zip code.
                        eprintln!("standardize_address: bad request: re-fetching without zip...");
                        adr.zip = "".into();
                        return self.standardize_address_inner(adr).await;
                    }
                }
                Err(err.into())
            }
    ...
    fn standardize_address_inner<'a>(
        &'a mut self,
        adr: &'a mut Address,
    ) -> Pin<Box<dyn Future<Output = Result<()>> + 'a>> {
        Box::pin(async move {
            self.standardize_address(adr).await?;
            Ok(())
        })
    }
}
